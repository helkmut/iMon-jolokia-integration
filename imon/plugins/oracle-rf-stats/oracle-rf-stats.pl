...
# Description: Monitor Oracle FusionMiddleware Reports and Forms Events and threads
#
#
# Author:
...
$ENV{"PATH"}="/u01/app/jdk/jdk1.7.0_40/bin:/u01/app/oracle/Middleware/asinst_1/config/reports/bin:/u01/app/oracle/Middleware/asinst_1/bin:/usr/lib64/qt-3.3/bin:/usr/local/bin:/bin:/usr/bin:/usr/local/sbin:/usr/sbin:/sbin";
...
sub check_connections {
	...
	my $command = `\$\(which netstat\) -an | grep $port | grep -v -i listen | wc -l`;
	chomp($command);
...
sub check_port_alive {
        ...
        my $command = `\$\(which netstat\) -an | grep $port | grep -i listen | wc -l`;
	chomp($command);
...
sub printHelp {
                ...
                This is a agent to check objects on Oracle Fusion Middleware Reports and Forms
		...
		-T  : Type of object
                ...
                E.g: $path/oracle-rf-stats.pl -v 1
